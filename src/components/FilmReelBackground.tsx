...
export default function FilmReelBackground({ photos = [], onPhotoClick }: FilmReelBackgroundProps) {
  // Extract image URLs or fall back to default rich set
  const imageUrls = useMemo(() => {
    if (photos && photos.length > 0) {
      const urls = photos.map((p) => p.image_url);
      return [...urls, ...urls, ...urls, ...DEFAULT_FILM_IMAGES];
    }
    return [...DEFAULT_FILM_IMAGES, ...DEFAULT_FILM_IMAGES, ...DEFAULT_FILM_IMAGES];
  }, [photos]);

  // Split into two tracks
  const track1 = useMemo(() => imageUrls.slice(0, Math.ceil(imageUrls.length / 2)), [imageUrls]);
  const track2 = useMemo(() => imageUrls.slice(Math.ceil(imageUrls.length / 2)), [imageUrls]);

  // Double tracks to ensure continuous 100% infinite marquee loop
  const loopTrack1 = useMemo(() => [...track1, ...track1], [track1]);
  const loopTrack2 = useMemo(() => [...track2, ...track2], [track2]);
  ...
  return (
    <div className="absolute inset-0 overflow-hidden select-none z-[5] pointer-events-auto">
      ...
      {/* Film Strip Row 1 — Positioned at top-10 sm:top-14 */}
      <div className="absolute top-10 sm:top-14 inset-x-0 overflow-visible py-1 z-10 transform -rotate-1 pointer-events-auto">
        <div className="film-track animate-film-left flex items-center gap-6 px-4 pointer-events-auto">
          {loopTrack1.map((url, idx) => (
            <div
              key={`t1-${idx}`}
              onClick={() => onPhotoClick?.(idx % (photos.length || 1))}
      ...
      {/* Film Strip Row 2 — Positioned higher up at top-[38%] sm:top-[40%] */}
