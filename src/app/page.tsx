...
export default function HomePage() {
  ...
  return (
    <div className="space-y-8 sm:space-y-10 pb-12 bg-transparent text-[var(--text-primary)]">
      ...
      <section className="relative min-h-[750px] sm:min-h-[820px] py-12 sm:py-16 flex flex-col justify-center items-center overflow-hidden pt-20 hero-backlight">
        ...
        >
          ...
          {/* Legacy Chip Tag */}
          <motion.div variants={itemVariants} className="relative z-20 mb-2 -translate-y-2.5 sm:-translate-y-3.5 inline-flex items-center gap-2.5 px-5 py-2 rounded-xl bg-[var(--bg-surface)]/95 border border-[var(--gold-primary)]/50 shadow-2xl text-xs font-extrabold text-[var(--gold-primary)] backdrop-blur-xl pointer-events-auto">
          ...
          <motion.div variants={itemVariants} className="relative w-full max-w-lg mx-auto py-2 flex items-center justify-center pointer-events-auto">
            ...
            <div className="absolute w-[240px] h-[240px] sm:w-[310px] sm:h-[310px] rounded-full bg-[var(--bg-dark)] shadow-[0_20px_50px_35px_var(--bg-dark)] pointer-events-none z-0" />
          ...
          {/* Headline & Narrative */}
          <motion.div variants={itemVariants} className="space-y-4 max-w-5xl mx-auto pointer-events-auto">
            <h1 className="font-serif-heading text-3xl sm:text-5xl md:text-6xl lg:text-7xl font-bold tracking-tight text-[var(--text-primary)] leading-tight whitespace-nowrap">
              Leadership Through <span className="text-gold-gradient">Service</span>
            </h1>
            <p className="font-sans-body text-[var(--text-secondary)] text-base sm:text-lg font-normal leading-relaxed max-w-4xl mx-auto">
              Welcome to the heartbeat of youth leadership and civic action in Navi Mumbai. <br className="hidden md:inline" />
              For 45 years, we have empowered youth to drive real community change, <br className="hidden md:inline" />
              turning shared passion into lasting social impact and lifelong fellowship.
            </p>
          </motion.div>
                  ...
                  <div className="p-5 sm:p-6 rounded-2xl modern-card-gold text-left relative overflow-hidden border border-[#d4af37]/40 shadow-2xl">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                      <div className="space-y-2">
                        ...
                        <h3 className="font-serif-heading text-xl sm:text-2xl font-bold text-[var(--text-primary)] leading-snug">
                          {upcoming.title}
                        </h3>
                ...
                <motion.button
                  ...
                  className={`w-full p-5 rounded-2xl text-left transition-all duration-300 flex items-center justify-between border ${
                    activeInitiativeIdx === idx
                      ? 'bg-gradient-to-r from-[var(--gold-subtle)] via-[var(--bg-card)] to-[var(--bg-surface)] border-[var(--gold-primary)] text-[var(--text-primary)] shadow-xl'
                      : 'bg-[var(--bg-dark)] border-[var(--border-primary)] text-[var(--text-muted)] hover:text-[var(--gold-primary)] hover:bg-[var(--bg-card)]'
                  }`}
                >
                  ...
                  <ChevronRight className={`w-5 h-5 transition-transform duration-300 ${activeInitiativeIdx === idx ? 'text-[#d4af37] translate-x-1' : 'text-[var(--text-muted)]'}`} />
                </motion.button>
              ))}
            </div>
            ...
            <div className="lg:col-span-7 space-y-6">
              ...
              <AnimatePresence mode="wait">
                <motion.div
                  ...
                  className="space-y-3"
                >
                  <h3 className="font-serif-heading text-3xl font-bold text-[var(--text-primary)]">
      ...
      {/* ═══════════════════════════════════════════════════════════════ */}
      <section className="py-8 sm:py-10 bg-[var(--bg-surface)] border-y border-[var(--border-primary)] relative overflow-hidden gradient-mesh-bg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
          ...
          >
            ...
            <Link
              href="/events"
              className="px-6 py-3 rounded-xl bg-[var(--bg-card)] border border-[var(--border-secondary)] hover:border-[var(--gold-primary)] text-xs font-bold text-[var(--text-primary)] flex items-center gap-2 hover:bg-[var(--gold-subtle)] transition-all self-start md:self-auto uppercase tracking-wider group"
            >
              ...
              >
                ...
                <div className="relative h-64 overflow-hidden rounded-t-[20px]">
                  ...
                  <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between text-xs text-[var(--text-secondary)]">
                    <span className="flex items-center gap-1.5 bg-black/80 px-3 py-1 rounded-md backdrop-blur-md text-[11px] font-semibold border border-[var(--border-secondary)]">
                      <Calendar className="w-3.5 h-3.5 text-[#d4af37]" />
                ...
                <div className="p-6 space-y-4 flex-1 flex flex-col justify-between">
                  <div className="space-y-2">
                    <h3 className="font-serif-heading text-2xl font-bold text-[var(--text-primary)] group-hover:text-[var(--gold-primary)] transition-colors duration-300 leading-snug">
                      {event.title}
                    </h3>
                  ...
                  {event.impact_metrics && event.impact_metrics.length > 0 && (
                    <div className="grid grid-cols-2 gap-2 pt-3 border-t border-[var(--border-primary)]">
                      {event.impact_metrics.slice(0, 2).map((m: { label: string; value: string }, metricIdx: number) => (
                        <div key={metricIdx} className="bg-[var(--bg-card)] p-2.5 rounded-xl text-center border border-[var(--border-primary)]">
                          <span className="font-serif-heading font-bold text-[#d4af37] text-base block">{m.value}</span>
                          <span className="text-[10px] text-[var(--text-muted)] uppercase block font-sans">{m.label}</span>
                        </div>
                      ))}
                    </div>
                  )}
                  ...
                  <div className="pt-3">
                    <Link
                      ...
                      className="w-full text-center py-3 rounded-xl bg-[var(--bg-card)] hover:bg-[var(--gold-primary)] hover:text-black font-bold text-xs transition-all duration-300 border border-[var(--border-secondary)] hover:border-[var(--gold-primary)] block uppercase tracking-wider"
                    >
          ...
          {photos.slice(0, 6).map((photo, idx) => {
            ...
            return (
              <motion.div
                ...
                className={`relative rounded-2xl overflow-hidden group cursor-pointer border border-[var(--border-secondary)] hover:border-[#d4af37]/60 transition-all shadow-xl hover:shadow-[0_0_30px_rgba(212,175,55,0.15)] ${
                  isTall ? 'row-span-2' : ''
                } ${isWide ? 'col-span-2' : ''}`}
              >
                {/* eslint-disable-next-line @next/next/no-img-element */}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center relative z-10">
              <div className="space-y-6">
                ...
                <div className="space-y-3">
                  <h3 className="font-serif-heading text-3xl sm:text-5xl font-bold text-[var(--text-primary)] leading-tight">
                ...
                <div className="flex flex-wrap items-center gap-4 text-xs text-[var(--text-muted)]">
                  <span>Author: <strong className="text-[var(--text-primary)]">{latestEditorial.author}</strong></span>
                  <span>•</span>
                  <span>Published: <strong className="text-[var(--text-primary)]">{latestEditorial.published_at && formatDate(latestEditorial.published_at)}</strong></span>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-4 pt-2">
                  ...
                  <Link
                    ...
                    className="px-6 py-4 btn-outline-action text-[var(--text-primary)] font-bold text-xs uppercase tracking-wider transition-all"
                  >
              ...
              <motion.div
                ...
                className="relative aspect-square rounded-2xl overflow-hidden border-2 border-[var(--border-secondary)] shadow-2xl hover:border-[#d4af37]/40 transition-colors"
              >
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
                ...
                <div className="absolute bottom-6 left-6 right-6 text-xs text-[var(--text-secondary)] space-y-1">
